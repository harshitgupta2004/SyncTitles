...
def parse_args():
    ...
    parser = srt_tools.utils.basic_parser(
        description=__doc__, examples=examples)
    parser.add_argument(
        "--from-start",
        "--f1",
...
def auto_sub_sync(start_frac, video_file, sub_file):
    ...
    while confidence*100 < int(values["-SC-"]) or not lineFound:
        ...
        try:
            ...
            result = rec.recognize_google(
                recAudio, language=values["-language-"], show_all=True)
            print(result)
            ...
            if len(text.split()) >= min_words:
                ...
                for line in subList:
                    ...
                    if values['-MSUB-'] == False:
                        ...
                        match = SequenceMatcher(None, text, subText).find_longest_match(
                            0, len(text), 0, len(subText))
    ...
    subTime = srt.timedelta_to_srt_timestamp(
        lineMatch.start + ((lineMatch.end - lineMatch.start) * indFrac) - ((lineMatch.end - lineMatch.start) * indFrac2))
...
if os.path.exists("savedtimes.txt"):
    ...
    times_list = ['00:00:00,000', '00:00:00,000',
...
srt_column = [
    ...
    [sg.Text(text="First SRT time: "), sg.InputText(
        key='-F1-', default_text=times_list[0])],
    [sg.Text(text="First Video time: "), sg.InputText(
        key='-T1-', default_text=times_list[1])],
    [sg.Text(text="Second SRT time: "), sg.InputText(
        key='-F2-', default_text=times_list[2])],
    [sg.Text(text="Second Video time: "), sg.InputText(
        key='-T2-', default_text=times_list[3])],
    [sg.Text(text="Encoding: "), sg.DropDown(
        ['utf-8', 'latin-1'], default_value='utf-8', key='-encoding-')],
    [sg.Button("OK", key='-SYNC-')],
    [sg.Text(size=(60, 3), key="-TOUT2-")],
    [sg.Text(text="AUTO: ")],
    [sg.Button("Auto", key="-AUTO-")],
    [sg.Text(size=(60, 2), key="-TOUT3-")],
    [sg.Text(text="Language: "), sg.DropDown(
        ['en-US', 'en-GB', 'fr-FR'], default_value='en-US', key='-language-')],
    [sg.Text(text="Speech Confidence: "), sg.InputText(
        key='-SC-', default_text='70')],
    [sg.Text(text="Minimum words to match: "), sg.DropDown(['1', '2', '3', '4', '5'],
                                                           default_value='3', key='-words-'), sg.Checkbox('Match substrings', key='-MSUB-')],
